...
import { createNavigation } from './navigation.js?v=20260902h';
...
} from './config.js?v=20260902h';
const firebase = globalThis.firebase;
(() => {
  let agePresenceCheckTimer=null,whoamiVoteInFlight=false,anonymousPersistenceReady=null;
  ...
  async function manualReconnect(){
    ...
    state.roomConnectionPaused=true;setConnectionStatus('reconnecting','Reconectando…');
    ...
    else{setConnectionStatus('reconnecting','Reconectando…');showReconnectModal('manual-failed');}
  ...
  async function acceptRoomInvite(roomCode){
    ...
    try{
      const roomSnapshot=await db.ref(`rooms/${roomCode}`).once('value');if(!roomSnapshot.exists()){await ignoreRoomInvite(roomCode);showSocialToast('La sala ya no existe.');return;}
      const invitedType=miniRoomType(roomSnapshot.val()),route=routeRoomByGameType(invitedType),declaredType=String(invite.gameType||'').trim().toLowerCase();
  ...
  function miniRoomType(data){
    const candidate=String(data?.gameType||data?.settings?.gameType||data?.game?.gameType||'').trim().toLowerCase();
    return ONLINE_GAME_TYPES.has(candidate)?candidate:'';
  }
  ...
  async function joinMiniRoom(type){
    ...
    try{
      ...
      const snapshot=await state.roomRef.once('value');if(!snapshot.exists()){state.roomRef=null;state.roomCode='';state.mode=null;notice('No existe una sala con ese código.','error',noticeId);return;}
      const data=snapshot.val(),actualType=miniRoomType(data);if(actualType!==type){state.roomRef=null;state.roomCode='';state.mode=null;notice(`Esa sala está configurada para ${MINI_GAME_LABELS[actualType]||'otro minijuego'}.`,'error',noticeId);return;}
  ...
  async function ensureTribunalPersonalScore(data){
    ...
    const resultWrite=await state.roomRef.child('game').transaction(current=>{
      ...
      scores[state.playerId]=(Number(scores[state.playerId])||0)+points;
       return {...current,scores,tribunal:{...tribunalPublicState(current.tribunal),scoredBy:{...(current.tribunal?.scoredBy||{}),[state.playerId]:true},roundPoints:{...(current.tribunal?.roundPoints||{}),[state.playerId]:points}}};
    });
  ...
  function renderTribunalResult(data){const game=data.game||{},result=game.roundResults||{},names=game.playerNames||{},players=miniPlayers(data),reveals=state.tribunalRevealData||{},complete=tribunalRevealComplete(game,reveals);setText('tribunalResultRoundLabel','CASO '+game.currentRound+' DE '+game.totalRounds);const accused=$('tribunalResultAccused');if(accused)accused.innerHTML=result.accusedId?'🚨 EL ACUSADO ERA:<br><strong>'+escapeHtml(cleanUiText(names[result.accusedId]||miniPlayerName(data,result.accusedId)))+'</strong>':'🔎 REVELANDO LOS ROLES…';const verdict=$('tribunalResultVerdict');if(verdict){const hasVerdict=typeof result.guilty==='boolean';verdict.className='tribunal-result-verdict '+(hasVerdict?(result.guilty?'guilty':'acquitted'): '');verdict.textContent=hasVerdict?(result.guilty?'⚖️ CULPABLE':'⚖️ ABSUELTO'):'⏳ ESPERANDO LA REVELACIÓN';}const table=$('tribunalResultScores');if(table)table.innerHTML=players.map(player=>{const role=reveals?.[player.id]?.role||'';return '<div class="tribunal-score-row"><strong>'+escapeHtml(cleanUiText(player.name))+'</strong><span>'+escapeHtml(role?tribunalRoleLabel(role):'🔒 ROL PENDIENTE')+'</span><b>+'+Number(game.tribunal?.roundPoints?.[player.id]||0)+' pts</b></div>';}).join('');const next=$('tribunalNextRoundBtn');if(next){const last=Number(game.currentRound)>=Number(game.totalRounds);next.classList.toggle('hidden',last);next.disabled=!isTribunalJudge()||!complete||typeof result.guilty!=='boolean';}const finish=$('tribunalFinishBtn');if(finish)finish.disabled=!isTribunalJudge()||!complete||typeof result.guilty!=='boolean';setText('tribunalResultStatus',!complete?'Esperando la revelación privada de todos los jugadores…':isTribunalJudge()?'El Juez puede continuar al siguiente caso.':'Esperando al Juez…');if(complete)void ensureTribunalPersonalScore(data);}
  ...
  async function advanceTribunalPresentation(){if(!state.roomRef||!isTribunalJudge())return false;const result=await state.roomRef.child('game').transaction(current=>{if(!current||current.phase!=='tribunalPresentation')return;return {...current,phase:'tribunalDebate',tribunal:{...tribunalPublicState(current.tribunal),estado:'debate'}};});if(result.committed)handleMiniRoomSnapshot({...state.lastRoomData,game:result.snapshot.val()});return Boolean(result.committed);}
  async function advanceTribunalDebate(){if(!state.roomRef||!isTribunalJudge())return false;const data=state.lastRoomData||{},game=data.game||{},assignment=tribunalPrivateAssignment(game),caso=assignment?.case||{},candidates=Object.keys(game.activePlayers||{}).filter(id=>id!==String(state.playerId)),text=Array.isArray(caso.evidenciasSorpresa)&&caso.evidenciasSorpresa.length?caso.evidenciasSorpresa[Math.floor(Math.random()*caso.evidenciasSorpresa.length)]:'';if(!candidates.length||!text)return false;const recipient=candidates[Math.floor(Math.random()*candidates.length)],next={...game,phase:'tribunalSurprise',tribunal:{...tribunalPublicState(game.tribunal),estado:'evidencia_sorpresa',surpriseDelivered:true}};const updates={};updates['rooms/'+state.roomCode+'/game']=next;updates['privateAssignments/'+state.roomCode+'/'+recipient+'/surprise']={round:game.currentRound,roundToken:game.roundToken,text};await db.ref().update(updates);return true;}
  async function advanceTribunalSurprise(){if(!state.roomRef||!isTribunalJudge())return false;const result=await state.roomRef.child('game').transaction(current=>{if(!current||current.phase!=='tribunalSurprise')return;return {...current,phase:'tribunalFinal',tribunal:{...tribunalPublicState(current.tribunal),estado:'final_juicio'}};});if(result.committed)handleMiniRoomSnapshot({...state.lastRoomData,game:result.snapshot.val()});return Boolean(result.committed);}
  async function advanceTribunalFinal(){if(!state.roomRef||!isTribunalJudge())return false;const result=await state.roomRef.child('game').transaction(current=>{if(!current||current.phase!=='tribunalFinal')return;return {...current,phase:'tribunalVoting',tribunal:{...tribunalPublicState(current.tribunal),estado:'votacion',votes:{}}};});if(result.committed)handleMiniRoomSnapshot({...state.lastRoomData,game:result.snapshot.val()});return Boolean(result.committed);}
  async function nextTribunalRound(){if(!state.roomRef||!isTribunalJudge())return false;const data=(await state.roomRef.once('value')).val(),game=data?.game||{};if(game.phase!=='tribunalResult')return false;const revealSnapshot=await db.ref(tribunalRevealPath(game.roundToken)).once('value'),reveals=revealSnapshot.val()||{};state.tribunalRevealData=reveals;if(!tribunalRevealComplete(game,reveals)||typeof game.roundResults?.guilty!=='boolean'){miniNotice('tribunalResultStatus','Esperando a que todos los roles sean revelados.','error');renderTribunalResult({...data,game});return false;}if(Number(game.currentRound)>=Number(game.totalRounds)){const result=await state.roomRef.child('game').transaction(current=>!current||current.phase!=='tribunalResult'?undefined:{...current,phase:'tribunalFinalResult',tribunal:{...tribunalPublicState(current.tribunal),estado:'resultado_final'}});if(result.committed)handleMiniRoomSnapshot({...state.lastRoomData,game:result.snapshot.val()});return Boolean(result.committed);}return startTribunalRound(data,miniPlayers(data));}
  ...
  function renderAgeReveal(data){
    if(state.ageLocal){const game=state.ageLocal,player=game.players?.[game.playerIndex],target=game.targets?.[player?.id];setText('ageRevealRoundLabel',`RONDA ${game.round} DE ${game.totalRounds} · ${player?.name||'Jugador'}`);setText('ageRevealInstruction','Mira tu edad en privado y luego entrega el teléfono.');setText('ageRevealNumber',isValidAgeTarget(target)?target:'—');setText('ageRevealCountdownLabel','REVEAL PRIVADO');setText('ageRevealCountdown','—');$('ageLocalRevealContinueBtn')?.classList.remove('hidden');return;}
    $('ageLocalRevealContinueBtn')?.classList.add('hidden');setText('ageRevealInstruction','Esta es tu edad. Muéstrala a los demás.');setText('ageRevealCountdownLabel','REVEAL');const game=data.game||{},target=ageTargetForPlayer(game,data,state.playerId);setText('ageRevealRoundLabel',`RONDA ${game.currentRound} DE ${game.totalRounds}`);setText('ageRevealNumber',isValidAgeTarget(target)?target:'—');setText('ageRevealCountdown',Math.max(0,Math.ceil(Math.max(0,Number(game.revealEndsAt)-serverNow())/1000)));
  }
  ...
  function createWhoamiLocalState(){return {players:[{id:'local-whoami-1',name:'Jugador 1'},{id:'local-whoami-2',name:'Jugador 2'}],categories:[],totalRounds:3,round:0,phase:'setup',revealIndex:0,assignments:{},publicCharacters:[],usedKeys:[],scores:{},roundResults:null};}
  ...
  function openWhoamiLocalSetup(){state.gameType=GAME_TYPES.WHOAMI;state.configEditing=false;state.whoamiLocal=createWhoamiLocalState();setWhoamiSetupMode(true);renderCategories();setCategorySelection([]);setTotalRounds(3);show('setup');}
  ...
  function startWhoamiLocal(){
    ...
    game.round=1;game.phase='preparing';game.revealIndex=0;game.scores=Object.fromEntries(game.players.map(player=>[player.id,0]));game.roundResults=null;state.whoamiLocal=game;state.gameType=GAME_TYPES.WHOAMI;state.mode='local';state.roomRef=null;state.roomCode='';state.lastRoomData=whoamiLocalSnapshot();show('prep');renderPreparation(state.lastRoomData.game);void requestScreenWakeLock();void requestAgeOrientationLock();startSynchronizedCountdown({phase:'localWhoamiPreparation',round:game.round,endsAt:serverNow()+PREPARATION_DURATION_MS,elementId:'prepCountdown',onZero:startWhoamiLocalReveal});
  }
  function startWhoamiLocalReveal(){const game=state.whoamiLocal;if(!game)return;clearCountdown();game.phase='reveal';game.revealIndex=0;state.lastRoomData=whoamiLocalSnapshot();show('reveal');renderReveal(state.lastRoomData.game);void requestScreenWakeLock();}
  function continueWhoamiLocalReveal(){
    const game=state.whoamiLocal;if(!game||game.phase!=='reveal')return;game.revealIndex++;
    if(game.revealIndex<game.players.length){state.lastRoomData=whoamiLocalSnapshot();renderReveal(state.lastRoomData.game);return;}
    game.phase='playing';state.lastRoomData=whoamiLocalSnapshot();show('playing');renderPlaying(state.lastRoomData.game,state.lastRoomData);void releaseScreenWakeLock();
  }
  ...
  function nextWhoamiLocalRound(){
    ...
    if(!prepareWhoamiLocalRound())return;game.round++;game.revealIndex=0;game.phase='preparing';game.roundResults=null;state.lastRoomData=whoamiLocalSnapshot();show('prep');renderPreparation(state.lastRoomData.game);void requestScreenWakeLock();startSynchronizedCountdown({phase:'localWhoamiPreparation',round:game.round,endsAt:serverNow()+PREPARATION_DURATION_MS,elementId:'prepCountdown',onZero:startWhoamiLocalReveal});
  }
  ...
  // ============================================================
  function localPlayerNames(listId,game,onChange){
    ...
    (game.players||[]).forEach((player,index)=>{
      ...
      const input=document.createElement('input');input.className='input';input.maxLength=30;input.value=player.name;input.placeholder=`Jugador ${index+1}`;input.setAttribute('aria-label',`Nombre del jugador ${index+1}`);input.addEventListener('input',event=>{player.name=String(event.target.value||'').slice(0,30);onChange?.();});
      const remove=document.createElement('button');remove.className='mini-remove';remove.type='button';remove.textContent='×';remove.setAttribute('aria-label',`Eliminar jugador ${index+1}`);remove.disabled=game.players.length<=2;remove.addEventListener('click',()=>{game.players.splice(index,1);onChange?.();});row.append(input,remove);list.appendChild(row);
    });
  }
  ...
  function createAgeLocalState(){return {players:[{id:'local-age-1',name:'Jugador 1'},{id:'local-age-2',name:'Jugador 2'}],totalRounds:Math.max(1,Number(ageData.defaultRounds)||3),round:0,phase:'setup',targets:{},estimates:{},scores:{},playerIndex:0,roundResults:null,prepEndsAt:null};}
  ...
  function startAgeLocal(){
    ...
    game.round=1;game.phase='preparing';game.playerIndex=0;game.scores=Object.fromEntries(game.players.map(player=>[player.id,0]));game.roundResults=null;game.targets=generateAgeTargets(game.players);game.estimates={};game.prepEndsAt=serverNow()+AGE_PREPARATION_DURATION_MS;state.ageLocal=game;state.gameType=GAME_TYPES.AGE;state.mode='local';state.roomRef=null;state.roomCode='';state.lastRoomData=ageLocalSnapshot();show('agePreparation');renderAgePreparation(state.lastRoomData);void requestScreenWakeLock();void requestAgeOrientationLock();startMiniCountdown({phase:'localAgePreparation',round:game.round,endsAt:game.prepEndsAt,elementId:'agePrepCountdown',onTick:seconds=>renderAgePreparation(state.lastRoomData,seconds),onZero:startAgeLocalReveal});
  }
  ...
  function continueAgeLocalReveal(){
    const game=state.ageLocal;if(!game||game.phase!=='reveal')return;game.phase='playing';state.ageLocalEstimate='';state.lastRoomData=ageLocalSnapshot();const input=$('ageEstimateInput');if(input)input.value='';show('agePlaying');renderAgePlaying(state.lastRoomData);void releaseScreenWakeLock();
  }
  ...
  function nextAgeLocalRound(){
    ...
    game.round++;game.phase='preparing';game.playerIndex=0;game.targets=generateAgeTargets(game.players);game.estimates={};game.roundResults=null;game.prepEndsAt=serverNow()+AGE_PREPARATION_DURATION_MS;state.lastRoomData=ageLocalSnapshot();show('agePreparation');renderAgePreparation(state.lastRoomData);void requestScreenWakeLock();startMiniCountdown({phase:'localAgePreparation',round:game.round,endsAt:game.prepEndsAt,elementId:'agePrepCountdown',onTick:seconds=>renderAgePreparation(state.lastRoomData,seconds),onZero:startAgeLocalReveal});
  }
  ...
  function openConfessionsSetup(){
    state.confessionsLocal=null;state.gameType=GAME_TYPES.CONFESSIONS;show('confessionsMode');
  }
  function openConfessionsOnlineSetup(){
    state.confessionsLocal=null;state.gameType=GAME_TYPES.CONFESSIONS;$('confessionsHostNameInput').value=accountUid()?accountUsername():state.playerName||'';document.querySelector('input[name="confessionsRoundsMode"][value="perPlayer"]')?.click();setConfessionsSetupMode(false);show('confessionsSetup');
  }
  ...
  async function joinAnyRoom(){
    ...
    try{
      ...
      const snapshot=await db.ref(`rooms/${code}`).once('value');
       if(!snapshot.exists()){state.roomRef=null;state.roomCode='';state.mode=null;notice('No existe una sala con ese código. Verifica el código.','error','joinNotice');return;}
       const type=miniRoomType(snapshot.val()),route=routeRoomByGameType(type);
  ...
  async function publishTribunalReveal(data){
    ...
    try{
      ...
      await revealRef.transaction(current=>current||payload);
  ...
  function listenToTribunalReveal(data){
    ...
    const revealRef=db.ref(tribunalRevealPath(token)),listener=snapshot=>{state.tribunalRevealData=snapshot.val()||{};renderTribunalResult(state.lastRoomData||data);void maybeFinalizeTribunalReveal(state.lastRoomData||data);};
  ...
  async function maybeFinalizeTribunalReveal(data){
    ...
    if(!state.roomRef||game.phase!=='tribunalResult'||!token||state.tribunalRevealFinalizeInFlight||!tribunalRevealComplete(game,reveals)||game.roundResults?.accusedId)return false;
    state.tribunalRevealFinalizeInFlight=true;
    try{
      const ids=tribunalActiveIds(game),accusedId=ids.find(id=>String(reveals[id]?.role||'')==='acusado');if(!accusedId)return false;
      ...
      const ordered=ids.map(id=>({playerId:id,votes:counts[id]||0})).sort((a,b)=>b.votes-a.votes||a.playerId.localeCompare(b.playerId)),guilty=ordered[0]?.playerId===String(accusedId);
      const result=await state.roomRef.child('game').transaction(current=>{if(!current||current.phase!=='tribunalResult'||String(current.roundToken||'')!==token||current.roundResults?.accusedId)return;return {...current,roundResults:{...current.roundResults,accusedId:String(accusedId),voteCounts:counts,guilty,verdict:guilty?'CULPABLE':'ABSUELTO'},tribunal:{...tribunalPublicState(current.tribunal),estado:'resultado',rolesRevealedAt:firebase.database.ServerValue.TIMESTAMP}};});
  ...
  function scheduleAutoReconnect(reason='connection-lost'){
    ...
    if(attempt>=state.reconnectMaxAttempts){if(!state.reconnectModalOpen)showReconnectModal('manual-failed');return;}
  ...
  async function joinRoom(){
    if([GAME_TYPES.AGE,GAME_TYPES.CONFESSIONS,GAME_TYPES.STOP,GAME_TYPES.CHAMUYA,GAME_TYPES.TRIBUNAL].includes(state.gameType)){await joinMiniRoom(state.gameType);return;}
    if(!ensureFirebaseConfigured())return;
    ...
    try{
      ...
      const snapshot=await state.roomRef.once('value');
      if(!snapshot.exists()){state.roomRef=null;state.roomCode='';state.mode=null;notice('No existe una sala con ese código. Verifica el código.','error','joinNotice');return;}
       const data=snapshot.val();if(miniRoomType(data)!==GAME_TYPES.WHOAMI){state.roomRef=null;state.roomCode='';state.mode=null;notice('Esa sala pertenece a otro minijuego. Usa TENGO UN CÓDIGO para que se detecte automáticamente.','error','joinNotice');return;}const players=normalizeRoomPlayers(data),currentPlayer=findAccountRoomPlayer(players),gamePhase=data.game?.phase||'lobby';
  ...
  async function startGame(){
    if([GAME_TYPES.AGE,GAME_TYPES.CONFESSIONS,GAME_TYPES.STOP,GAME_TYPES.CHAMUYA,GAME_TYPES.TRIBUNAL].includes(state.gameType)){await startMiniGame();return;}
    if(state.busy.start||!state.roomRef||state.mode!=='host')return;setBusy('start',true);setButtonBusy('startRoomBtn','start',true,'INICIANDO…');
    try{
  ...
  function renderReveal(game){
    if(state.whoamiLocal){
      const localGame=state.whoamiLocal,player=localGame.players?.[localGame.revealIndex],assignment=localGame.assignments?.[player?.id],wrap=document.querySelector('#reveal .countdown-wrap'),button=$('whoamiLocalRevealContinueBtn');
      setText('revealRoundLabel',`RONDA ${localGame.round} DE ${localGame.totalRounds} · ${player?.name||'Jugador'}`);setText('revealAudience','PANTALLA PRIVADA · NO MIRES');setText('revealCharacterName',assignment?.character?.nombre?cleanUiText(assignment.character.nombre):'Preparando carta…');setText('revealCharacterCategory',assignment?.character?.categoria?cleanUiText(assignment.character.categoria):'');renderCharacterImage('revealCharacterImageBox',assignment?.character||null);wrap?.classList.add('hidden');button?.classList.remove('hidden');if(button)button.textContent=localGame.revealIndex>=localGame.players.length-1?'TERMINAR REVEAL':`OCULTAR Y PASAR A ${String(localGame.players?.[localGame.revealIndex+1]?.name||'SIGUIENTE').toUpperCase()}`;return;
    }
    document.querySelector('#reveal .countdown-wrap')?.classList.remove('hidden');$('whoamiLocalRevealContinueBtn')?.classList.add('hidden');
  ...
  async function leaveRoom(preserveRoom=false){
    ...
    try{
      ...
      if(!preserveRoom&&ref){
        if(currentlyHost){await ref.remove();await Promise.all([db.ref(`privateAssignments/${roomId}`).remove(),db.ref(`tribunalReveals/${roomId}`).remove()]).catch(()=>{});}
        else if(currentPlayer)await ref.child('players/'+state.playerId).remove();
      }
    }catch(error){console.error('leaveRoom',error);}
    finally{stopRoomListener();stopAssignmentListeners();state.roomRef=null;state.lastRoomData=null;state.mode=null;state.gameType=null;state.roomCode='';state.myAssignment=null;state.miniRenderKey='';state.stopFormToken='';clearSession();$('joinRoomBtn').disabled=false;$('createRoomBtn').disabled=false;resetHistory();show('home',{history:false});showSocialToast(preserveRoom?'Saliste de la sala. Puedes volver con el código.':'Sala cerrada.');setBusy('leave',false);}
  }
  ...
  $('chamuyayaSalaBtn').onclick=()=>{haptic([18]);openChamuyayaOnlineSetup();};$('chamuyayaCelularBtn').onclick=()=>{haptic([18]);state.chamuyayaLocal=state.chamuyayaLocal||createChamuyayaLocalState();renderChamuyayaLocalSetup();show('chamuyayaSetup');};$('chamuyayaHomeBackBtn').onclick=()=>show('minigames',{history:false});
  $('chamuyayaCreateRoomBtn').onclick=()=>{state.gameType=GAME_TYPES.CHAMUYA;void createMiniRoom(GAME_TYPES.CHAMUYA);};$('chamuyayaJoinRoomBtn').onclick=()=>openGeneralJoin();$('chamuyayaOnlineBackBtn').onclick=()=>show('chamuyayaHome',{history:false});
  ...
  $('ageOnlineModeSelectBtn').onclick=()=>{haptic([18]);openAgeOnlineSetup();};$('ageLocalModeSelectBtn').onclick=()=>{haptic([18]);openAgeLocalSetup();};$('ageModeBackBtn').onclick=()=>{state.gameType=null;show('minigames');};$('ageRoundsMinusBtn').onclick=()=>updateAgeRounds(-1);$('ageRoundsPlusBtn').onclick=()=>updateAgeRounds(1);$('ageLocalRoundsMinusBtn').onclick=()=>updateAgeLocalRounds(-1);$('ageLocalRoundsPlusBtn').onclick=()=>updateAgeLocalRounds(1);$('ageOnlineModeBtn').onclick=()=>setAgeSetupMode(false);$('ageLocalModeBtn').onclick=()=>setAgeSetupMode(true);$('ageLocalAddPlayerBtn').onclick=()=>{const game=state.ageLocal||createAgeLocalState();if(game.players.length>=20){miniNotice('ageSetupNotice','Máximo 20 jugadores.','error');return;}game.players.push({id:`local-age-${game.players.length+1}`,name:`Jugador ${game.players.length+1}`});state.ageLocal=game;renderAgeLocalSetup();};$('ageLocalStartBtn').onclick=startAgeLocal;$('ageLocalRevealContinueBtn').onclick=continueAgeLocalReveal;$('ageLocalHandoffBtn').onclick=continueAgeLocalPlayer;$('ageCreateRoomBtn').onclick=()=>{state.gameType=GAME_TYPES.AGE;void createMiniRoom(GAME_TYPES.AGE);};$('ageJoinRoomBtn').onclick=openGeneralJoin;$('ageBackBtn').onclick=()=>{state.ageLocal=null;state.gameType=GAME_TYPES.AGE;show('ageMode');};$('ageSubmitBtn').onclick=()=>void submitAgeEstimate();$('ageEstimateInput').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();void submitAgeEstimate();}});
  $('confessionsOnlineModeSelectBtn').onclick=()=>{haptic([18]);openConfessionsOnlineSetup();};$('confessionsLocalModeSelectBtn').onclick=()=>{haptic([18]);openConfessionsLocalSetup();};$('confessionsModeBackBtn').onclick=()=>{state.gameType=null;show('minigames');};$('confessionsOnlineModeBtn').onclick=()=>setConfessionsSetupMode(false);$('confessionsLocalModeBtn').onclick=openConfessionsLocalSetup;$('confessionsLocalAddPlayerBtn').onclick=()=>{const game=state.confessionsLocal||createConfessionsLocalState();if(game.players.length>=20){miniNotice('confessionsSetupNotice','Máximo 20 jugadores.','error');return;}game.players.push({id:`local-conf-${game.players.length+1}`,name:`Jugador ${game.players.length+1}`});state.confessionsLocal=game;renderConfessionsLocalSetup();};$('confessionsLocalStartBtn').onclick=startConfessionsLocal;$('confessionsLocalHandoffBtn').onclick=continueConfessionsLocalWriting;$('confessionsLocalNextBtn').onclick=nextConfessionsLocalResult;$('confessionsLocalScoreboardNextBtn').onclick=nextConfessionsLocalScoreboard;$('confessionsCreateRoomBtn').onclick=()=>{state.gameType=GAME_TYPES.CONFESSIONS;void createMiniRoom(GAME_TYPES.CONFESSIONS);};$('confessionsJoinRoomBtn').onclick=openGeneralJoin;$('confessionsBackBtn').onclick=()=>{state.confessionsLocal=null;state.gameType=GAME_TYPES.CONFESSIONS;show('confessionsMode');};$('submitConfessionBtn').onclick=()=>void submitConfession();if($('confessionInput'))$('confessionInput').maxLength=confessionsMaxLength;$('confessionInput').addEventListener('input',event=>setText('confessionCharCount',`${String(event.target.value||'').length}/${confessionsMaxLength}`));$('confessionInput').addEventListener('keydown',event=>{if(event.key==='Enter'&&(event.ctrlKey||event.metaKey)){event.preventDefault();void submitConfession();}});
  $('stopRoundsSelect').addEventListener('change',()=>{const config=loadStopConfig();config.totalRounds=Number($('stopRoundsSelect').value)||3;saveStopConfig();});$('stopTimeSelect').addEventListener('change',()=>{const config=loadStopConfig();config.timeSeconds=Number($('stopTimeSelect').value)||60;saveStopConfig();});$('stopToggleLettersBtn').onclick=stopToggleLetters;$('stopToggleCategoriesBtn').onclick=stopToggleCategories;$('stopAddCategoryBtn').onclick=addStopCategory;$('stopCustomCategoryInput').addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();addStopCategory();}});$('stopCreateRoomBtn').onclick=()=>{state.gameType=GAME_TYPES.STOP;void createMiniRoom(GAME_TYPES.STOP);};$('stopJoinRoomBtn').onclick=openGeneralJoin;$('stopBackBtn').onclick=()=>{state.gameType=null;show('minigames');};$('stopButton').onclick=()=>void pressStop();$('stopFinishReviewBtn').onclick=()=>void finishStopReview();$('miniNextRoundBtn').onclick=()=>void nextMiniRound();$('miniFinishMenuBtn').onclick=()=>{if(state.ageLocal)finishAgeLocal();else if(state.confessionsLocal)finishConfessionsLocal();else if(state.roomRef)void requestLeaveRoom();else{resetHistory();show('home',{history:false});}};
   // Las acciones de regreso son transiciones contextuales y no agregan
   // pantallas internas al historial de navegación.
   $('ageModeBackBtn').onclick=()=>{state.gameType=null;show('minigames',{history:false});};
   $('ageBackBtn').onclick=()=>{state.ageLocal=null;state.gameType=GAME_TYPES.AGE;show('ageMode',{history:false});};
   $('confessionsModeBackBtn').onclick=()=>{state.gameType=null;show('minigames',{history:false});};
   $('confessionsBackBtn').onclick=()=>{state.confessionsLocal=null;state.gameType=GAME_TYPES.CONFESSIONS;show('confessionsMode',{history:false});};
   $('chamuyayaHomeBackBtn').onclick=()=>show('minigames',{history:false});
   $('chamuyayaOnlineBackBtn').onclick=()=>show('chamuyayaHome',{history:false});
   $('chamuyayaLocalBackBtn').onclick=()=>show('chamuyayaHome',{history:false});
  ...
  window.addEventListener('offline',()=>{if(state.roomRef){state.roomConnectionPaused=true;setConnectionStatus('reconnecting','Reconectando…');scheduleAutoReconnect('browser-offline');}});
  window.addEventListener('online',()=>{if(state.roomRef){state.roomConnectionPaused=true;setConnectionStatus('reconnecting','Reconectando…');scheduleAutoReconnect('browser-online');}});
