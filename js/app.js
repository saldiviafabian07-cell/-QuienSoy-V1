...
(() => {
  ...
  async function logoutAccount(){
    ...
    try{
      if(state.roomRef)await leaveRoom(true);
      stopAccountListeners();
      await auth.signOut();state.guestMode=false;sessionStorage.removeItem('qs_guest_auth_uid');closeQuickFriends();show('home');
    }
  ...
  function setWhoamiSetupMode(local){
    ...
    else{state.mode=null;state.whoamiLocal=null;}
  }
  ...
  function finishWhoamiLocal(){clearCountdown();clearMiniCountdown();state.whoamiLocal=null;state.lastRoomData=null;state.mode=null;state.gameType=GAME_TYPES.WHOAMI;show('minigames');}
  ...
  function finishAgeLocal(){clearMiniCountdown();state.ageLocal=null;state.lastRoomData=null;state.mode=null;state.gameType=GAME_TYPES.WHOAMI;show('minigames');}
  ...
  function finishConfessionsLocal(){state.confessionsLocal=null;state.lastRoomData=null;state.mode=null;state.gameType=GAME_TYPES.WHOAMI;show('minigames');}
  ...
  async function reconnectPlayer(roomId,playerName,playerId){
    ...
    try{
      ...
      const data=snapshot.val(),type=miniRoomType(data);if(!routeRoomByGameType(type)){clearSession();state.roomRef=null;state.lastRoomData=null;state.roomCode='';stopAssignmentListeners();notice('La sala tiene un tipo de juego desconocido. No se abrirá ninguna partida.','error','joinNotice');show(accountUid()?'mainMenu':'home');return false;}state.gameType=type;const matched=findAccountRoomPlayer(normalizeRoomPlayers(data));
  ...
  const { $, setText, show, goToScreenIfChanged, goBack, notice, clearNotice } = navigation;
  ...
  $('goCreateBtn').onclick=()=>{state.gameType=GAME_TYPES.WHOAMI;state.configEditing=false;$('hostNameInput').value=accountUid()?accountUsername():'';openSettings(false);};$('backHomeBtn').onclick=()=>state.configEditing?show('lobby'):state.whoamiLocal?(state.whoamiLocal=null,state.mode=null,show('gameHome')):leaveRoom();
  $('guestBtn').onclick=()=>void enterGuest();
  $('loginBtn').onclick=()=>{haptic([18]);show('authLogin');};
  $('registerBtn').onclick=()=>{haptic([18]);show('authRegister');};
  $('createAccountBtn').onclick=registerAccount;$('signInBtn').onclick=loginAccount;$('toLoginBtn').onclick=()=>show('authLogin');$('toRegisterBtn').onclick=()=>show('authRegister');$('registerBackBtn').onclick=()=>goBack(accountUid()?'mainMenu':'home');$('loginBackBtn').onclick=()=>goBack(accountUid()?'mainMenu':'home');
