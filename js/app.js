...
(() => {
  ...
  async function joinMiniRoom(type){
    ...
    try{
      ...
      if(!directory||declaredType!==type){state.roomRef=null;state.roomCode='';state.mode=null;notice(!directory?'No existe una sala con ese código. Verifica el código.':`Esa sala está configurada para ${MINI_GAME_LABELS[declaredType]||'otro minijuego'}.`,'error',noticeId);return;}
      ...
      if(phase!=='lobby'&&!reconnecting){notice('La partida ya comenzó. Espera a la próxima partida.','error',noticeId);return;}if(Number(directory.playerCount)>=ROOM_DIRECTORY_MAX_PLAYERS&&!reconnecting){notice('La sala está llena.','error',noticeId);return;}
  ...
  function renderTribunalResult(data){const game=data.game||{},result=game.roundResults||{},names=game.playerNames||{},players=miniPlayers(data),reveals=state.tribunalRevealData||{},complete=tribunalRevealComplete(game,reveals),revealFinished=complete||Boolean(result.revealTimedOut);setText('tribunalResultRoundLabel','CASO '+game.currentRound+' DE '+game.totalRounds);const accused=$('tribunalResultAccused');if(accused)accused.innerHTML=result.accusedId?'🚨 EL ACUSADO ERA:<br><strong>'+escapeHtml(cleanUiText(names[result.accusedId]||miniPlayerName(data,result.accusedId)))+'</strong>':result.revealTimedOut?'⚠️ REVELACIÓN INCOMPLETA':'🔎 REVELANDO LOS ROLES…';const verdict=$('tribunalResultVerdict');if(verdict){const hasVerdict=typeof result.guilty==='boolean';verdict.className='tribunal-result-verdict '+(hasVerdict?(result.guilty?'guilty':'acquitted'): '');verdict.textContent=hasVerdict?(result.guilty?'⚖️ CULPABLE':'⚖️ ABSUELTO'):result.revealTimedOut?'⚠️ REVELACIÓN INCOMPLETA':'⏳ ESPERANDO LA REVELACIÓN';}const table=$('tribunalResultScores');if(table)table.innerHTML=players.map(player=>{const role=reveals?.[player.id]?.role||'';return '<div class="tribunal-score-row"><strong>'+escapeHtml(cleanUiText(player.name))+'</strong><span>'+escapeHtml(role?tribunalRoleLabel(role):'🔒 ROL PENDIENTE')+'</span><b>+'+Number(game.tribunal?.roundPoints?.[player.id]||0)+' pts</b></div>';}).join('');const next=$('tribunalNextRoundBtn');if(next){const last=Number(game.currentRound)>=Number(game.totalRounds);next.classList.toggle('hidden',last);next.disabled=!isTribunalJudge()||!revealFinished||(!result.revealTimedOut&&typeof result.guilty!=='boolean');}const finish=$('tribunalFinishBtn');if(finish)finish.disabled=!isTribunalJudge()||!revealFinished||(!result.revealTimedOut&&typeof result.guilty!=='boolean);setText('tribunalResultStatus',!revealFinished?'Esperando la revelación privada de todos los jugadores…':result.revealTimedOut?'El tiempo de revelación terminó; puedes continuar.':isTribunalJudge()?'El Juez puede continuar al siguiente caso.':'Esperando al Juez…');if(complete)void ensureTribunalPersonalScore(data);}
  ...
  async function submitAgeEstimate(){
    ...
    try{
      const estimateRef=state.roomRef.child(`game/ageEstimates/${state.playerId}`),submittedRef=state.roomRef.child(`game/ageSubmitted/${state.playerId}`),estimateResult=await estimateRef.transaction(current=>current===null?value:current);
      ...
      const submittedResult=await submittedRef.transaction(current=>current===true?current:true);if(!submittedResult.committed||submittedResult.snapshot.val()!==true){miniNotice('agePlayingNotice','No se pudo confirmar tu estimación. Revisa la conexión.','error');return;}
  ...
  async function nextMiniRound(){
    ...
    try{
      const data=(await state.roomRef.once('value')).val(),type=miniRoomType(data),canAdvance=isHost(data);
  ...
  async function handleMiniRoomSnapshot(data){
    ...
    state.gameType=type;state.playerName=me.name||state.playerName;state.hostName=data.hostName||state.hostName;state.mode=isHost(data)?'host':'player';saveSession();if(state.mode==='host')void syncRoomDirectory(data);if(!isHost(data)&&data.hostId&&!isPlayerOnline(data.players?.[data.hostId]))void attemptHostTransfer(data);
  ...
  function handleRoomSnapshot(data){
    ...
    state.playerName=me.name||state.playerName;state.hostName=data.hostName||state.hostName;state.mode=isHost(data)?'host':'player';saveSession();if(state.mode==='host')void syncRoomDirectory(data);
  ...
  async function joinRoom(){
    ...
    try{
      ...
      const directory=await readRoomDirectory(code),declaredType=String(directory?.gameType||'').trim().toLowerCase();
      if(!directory||declaredType!==GAME_TYPES.WHOAMI){state.roomRef=null;state.roomCode='';state.mode=null;notice(!directory?'No existe una sala con ese código. Verifica el código.':'Esa sala pertenece a otro minijuego. Usa TENGO UN CÓDIGO para que se detecte automáticamente.','error','joinNotice');return;}
      ...
      if(Number(directory.playerCount)>=ROOM_DIRECTORY_MAX_PLAYERS&&!reconnecting){notice('La sala está llena.','error','joinNotice');return;}
      if(reconnecting){
    ...
    try{
      ...
      if(!preserveRoom&&ref){
        if(currentlyHost){
          ...
          if(hostTransferred){
            if(currentPlayer)await ref.child('players/'+state.playerId).update({status:'offline',connected:false,lastSeen:firebase.database.ServerValue.TIMESTAMP,leftAt:firebase.database.ServerValue.TIMESTAMP});
