...
(() => {
  ...
  async function startChamuyayaRound(data,players=chamuyayaRoundPlayers(data)){
    ...
    const config=data.settings?.chamuyaya||defaultChamuyayaConfig(),count=Math.max(1,Math.min(players.length-1,Number(config.chaMuyaCount)||1)),normalCount=players.length-count,game=data.game||{},used=Array.isArray(game.chamuyaya?.usedDataIds)?game.chamuyaya.usedDataIds.map(Number):[],available=chamuyayaCatalog.filter(item=>!used.includes(Number(item.id))),recycled=chamuyayaCatalog.filter(item=>used.includes(Number(item.id))),dataPool=shuffleArray(available).concat(shuffleArray(recycled)).slice(0,normalCount);
    if(dataPool.length<normalCount){notice('ChaMuYa2 necesita al menos un dato distinto por cada jugador normal. Agrega más datos en data/chamuyaya/data.js.','error','lobbyNotice');return false;}
    const round=Number(game.currentRound||0)+1,token=miniToken(GAME_TYPES.CHAMUYA,round),chaIds=shuffleArray(players.map(player=>String(player.id))).slice(0,count),assignments={};let dataIndex=0;
  ...
  function startChamuyayaLocalRound(){const game=state.chamuyayaLocal||createChamuyayaLocalState(),chaCount=Math.max(1,Math.min(game.players.length-1,Number(game.chaMuyaCount)||1));game.chaMuyaIds=shuffleArray(game.players.map(player=>player.id)).slice(0,chaCount);const normalPlayers=game.players.filter(player=>!game.chaMuyaIds.includes(player.id)),used=Array.isArray(game.usedDataIds)?game.usedDataIds.map(Number):[],available=chamuyayaCatalog.filter(item=>!used.includes(Number(item.id))),recycled=chamuyayaCatalog.filter(item=>used.includes(Number(item.id))),dataPool=shuffleArray(available).concat(shuffleArray(recycled)).slice(0,normalPlayers.length);if(dataPool.length<normalPlayers.length){miniNotice('chamuyayaLocalNotice','ChaMuYa2 necesita al menos un dato distinto por cada jugador normal. Agrega más datos en data/chamuyaya/data.js.','error');return;}game.chaMuyaCount=chaCount;game.dataIdsByPlayer={};normalPlayers.forEach((player,index)=>{game.dataIdsByPlayer[player.id]=Number(dataPool[index].id);});game.dataId=Number(dataPool[0]?.id||0);game.usedDataIds=[...new Set([...used,...dataPool.map(item=>Number(item.id))])];game.phase='reveal';game.revealIndex=0;game.revealVisible=false;game.discussionVisible=false;game.selectedVotes=[];game.voterIndex=0;game.votes=[];localChamuyayaSave();show('chamuyayaLocalReveal');renderChamuyayaLocalReveal();}
