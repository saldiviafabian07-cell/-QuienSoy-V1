...
import { createNavigation } from './navigation.js?v=20260902i';
...
(() => {
  ...
  async function maybeFinalizeWhatWouldYouDo(data){
    const game=data?.game||{};if(!state.roomRef||game.phase!=='whatWouldYouDoPlaying'||!whatWouldYouDoVotesComplete(game))return false;
    const result=await state.roomRef.child('game').transaction(current=>{
  ...
  function whatWouldYouDoQuestionText(game){const question=whatWouldYouDoQuestionFor(game);return question.optionA&&question.optionB?`¿Qué prefieres: ${question.optionA} o ${question.optionB}?`:'';}
  ...
  async function submitWhatWouldYouDoVote(choice){
    ...
    try{
      const result=await state.roomRef.child('game').transaction(game=>{if(!game||game.phase!=='whatWouldYouDoPlaying'||!game.activePlayers?.[ownId]||game.whatWouldYouDo?.votes?.[ownId])return;return {...game,whatWouldYouDo:{...game.whatWouldYouDo,votes:{...(game.whatWouldYouDo.votes||{}),[ownId]:choice}}};});
      if(result.committed){const nextData={...state.lastRoomData,game:result.snapshot.val()};state.lastRoomData=nextData;renderWhatWouldYouDoPlaying(nextData);void maybeFinalizeWhatWouldYouDo(nextData);}
